...
trait HasRelationship
{
    ...
    protected string $parent_column_name = "parent_id";
    ...
    public function relationship(string | Closure | null $name = null, ?Closure $modifyQueryUsing = null): static
    {
        ...
        $this->saveRelationshipsUsing(static function (AdjacencyList $component, ?array $state) {
            ...
            $parent_column_name = $component->getParentColumnName();

            Arr::map(
                $state,
                $traverse = function (array $item, string $itemKey, array $siblings = []) use (&$traverse, &$cachedExistingRecords, $state, $relationship, $childrenKey, $recordKeyName, $orderColumn, $pivotAttributes, $parent_column_name): Model {
                    $record = $relationship->getModel()::find($item[$recordKeyName]);
                    /* Update item order */
                    if ($orderColumn) {
                    ...
                    if ($relationship instanceof BelongsToMany) {
                        ...
                        $record->{$parent_column_name} = null;
    ...
    public function parentColumnName($column="parent_id"): static
    {
        $this->parent_column_name = $column;
        return $this;
    }
    ...
    public function getParentColumnName(){
        return $this->evaluate($this->parent_column_name);
    }
