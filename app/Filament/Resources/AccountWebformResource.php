...
class AccountWebformResource extends Resource implements HasShieldPermissions
{
    use HasForm, Translatable;

    protected static ?string $model = AccountWebform::class;
        ...
        return $table
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('account')
                    ->relationship('account', 'title->' . app()->getLocale())
                    ->searchable()
                    ->preload()
                    ->label(__("Account")),
                Tables\Filters\Filter::make('name')
                    ...
                    ->query(function (Builder $query, array $data){
                        $query->when(
                            $data['name'],
                            fn (Builder $builder, $name) => $query->where('name_ar', 'like', '%' . $name . '%')->orWhere('name_en', 'like', '%' . $name . '%'));
                        $query->when(
                            $data['email'],
                            fn (Builder $builder, $email) => $query->where('email', 'like', '%' . $email . '%'));
                        $query->when(
                            $data['phone_number'],
                            fn (Builder $builder, $phone) => $query
                                ->where('phone_number_1', 'like', '%' . $phone . '%')
                                ->orWhere('phone_number_2', 'like', '%' . $phone . '%')
                        );
                    })
            ])
            ->poll('30s')
            ->actions([
                Tables\Actions\Action::make('view')
                    ...
                    ->modalContent(function ($record){
                        return view('filament.Accounts.WebformView', ['record' => $record]);
                    })
                ...
                Tables\Actions\Action::make('document')
                    ->label(__("Document"))
                    ->url(function ($record){
                        return asset('/storage/forms/' . $record->cv);
    ...
    {
        return [
            'index' => \App\Filament\Resources\AccountWebformResource\Pages\ListAccountWebforms::route('/'),
        ];
    }
