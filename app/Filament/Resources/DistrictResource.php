...
use App\Filament\Resources\DistrictResource\Widgets\DistrictsWidget;
                            ...
                            foreach (config('app.locales') as $locale => $language){
                                $tabs[] = Forms\Components\Tabs\Tab::make($language)->schema([
                                    TextInput::make("title.{$locale}")
                                        ...
                                        ->languageRule()
                                        ->maxLength(255)
                                        ->unique(District::class, "title->{$locale}", ignoreRecord: true)
                                ]);
                            }
                            return $tabs;
                        })
                    ...
                    Forms\Components\Section::make()->schema(
                        array_merge(
                            ...
                            Forms\Components\Select::make('city_id')
                                ->relationship('city', 'title->' . config('app.locale'))
                                ->preload()
                                ->searchable(),
