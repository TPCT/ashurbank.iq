            ...
            ->schema([
                Forms\Components\Grid::make(3)->schema([
                                    ...
                                    foreach (config('app.locales') as $locale => $language) {
                                        $tabs[] = Forms\Components\Tabs\Tab::make($language)->schema([
                                            TextInput::make("title.{$locale}")
                                                ...
                                                ->unique(Card::class, "title->{$locale}", ignoreRecord: true)
                                                ->required(),
                    ...
                    Forms\Components\Grid::make(1)->schema([
                        Forms\Components\Section::make()->schema(
                            array_merge(
                                Filament::auth()->user()->can('change_author_card') ? [
                                ...
                                Forms\Components\Select::make('form_type')
                                    ->options(Card::getFormTypes())
                                    ->default(Card::PROGRESS_FORM)
                                    ->native(false)
                                    ->preload()
                                    ->required(),
                                Forms\Components\Checkbox::make('promote')
                                    ->label(__("Promote To Index"))
                                    ->default(true),
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('status')
                    ...
                    ->label(__("Status"))
                    ->badge()
                    ->color(function (Card $record) {
                        return $record->status == Utilities::PUBLISHED ? "success" : "danger";
                    })
                    ->formatStateUsing(function (Card $record) {
