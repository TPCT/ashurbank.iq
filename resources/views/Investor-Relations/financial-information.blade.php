...
@section('title', __('site.Financial Information'))
@section('id', 'FinancialStatements')

@section('content')
    <x-layout.header-image title="{{__('site.Financial Information')}}"></x-layout.header-image>

    @if ($financial_information_first_section)
        <section class="financialInformation-1stsection">
            <div class="container">
                <img src="{{asset('/storage/' . $financial_information_first_section->image)}}" alt="" srcset="">
                <div>
                    <h2>{{$financial_information_first_section->title}}</h2>
                    {!! $financial_information_first_section->description !!}
                </div>
            </div>
        </section>
    @endif
    ...
    <div id="items-section" class="mb-3">
        <section class="financialInformation-2ndsection">
            <div class="container ">
                @forelse ($financial_statements as $financial_statement)
                    <div class="financialInformation-card">
                        <h2>{{$financial_statement->title}}</h2>
                        <div>
                            <img src="{{asset('/images/icons/calendar.png')}}" alt="">
                            <p>{{\Carbon\Carbon::parse($financial_statement->published_at)->year}}</p>
                        </div>
                        <a class="d-flex justify-content-center testing" target="_blank" href="{{asset('/storage/' . $financial_statement->link)}}">
                            <button class="secondry-btn">
                                <img src="{{asset('/images/icons/Pdf File.png')}}" alt="" srcset="">@lang('site.Download')
                            </button>
                        </a>
                    </div>
                @empty
    ...
    <script>
        $(function(){
            $("#search-form").on('submit', function (e){
                e.preventDefault();
                $("#items-section").load('{{route('investor-relations.financial-statements')}}' + "?" + $(this).serialize() + " " + "#items-section");
            });
